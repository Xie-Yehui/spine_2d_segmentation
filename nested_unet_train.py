...
def main():
    ...
    if config['name'] is None:
        if config['deep_supervision']:
            config['name'] = '%s_%s_wDS' % (time_str, config['arch'])
        else:
            config['name'] = '%s_%s_woDS' % (time_str, config['arch'])
    os.makedirs('Result/%s' % config['name'], exist_ok=True)
    ...
    with open('Result/%s/config.yml' % config['name'], 'w') as f:
        yaml.dump(config, f)
    ...
    for epoch in range(config['epochs']):
        ...
        pd.DataFrame(log).to_csv('Result/%s/log.csv' % config['name'], index=False)

        trigger += 1

        if val_log['iou'] > best_iou:
            torch.save(model.state_dict(), 'Result/%s/best_model.pth' % config['name'])
